...
class NER(nn.Module):
    def __init__(self, dict_path, model_name, tag_value ,dropout = 0.4,  max_len = 256, batch_size = 32, device = 'cuda'):
    ...
    def predict(self, texts):
        ...
        for sub in sub_cut:
            ...
            tests, tags = utils.merge_subtags(sub, tags_predict, self.model_name)
            words_out += tests
            tags_out += tags
        out = [(w,t) if w != '.</s>' else  (w.replace('.</s>','[/n]'),t) for w,t in zip(words_out,tags_out)]
        # out = [(w,t) for w,t in zip(words_out,tags_out)]
        out = processing_data.post_processing(texts, out)
        return out
